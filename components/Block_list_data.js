...
const Block_list_data = ({item, cluster, userID, componentId, name, password, roundNo}) => {
...
const call=(block)=>{
    ...
    const block_show_details=()=>{
      ...
      db.transaction(tx=>{
        tx.executeSql(
          ...
          "AND "+ 
          "b.is_MWRA = '1' and "+ 
          "b.Cluster = '"+cluster+"' and b.Block='"+item.block_no+"' ",
        [],
        (tx, result)=>{          
      ...
      db.transaction(tx=>{
        tx.executeSql(
          ...
          "a.Mem_Cstatus in ('1') "+
          "and "+
          "a.Cluster = '"+cluster+"' and a.Block='"+item.block_no+"' ",
        [],
        (tx, result)=>{          
        var length = result.rows.length;          
                for(let i= 0; i<length; i++){
  ...
  return (
    ...
    <TouchableHighlight underlayColor={"#cffafe"} onPress={call.bind(this,item.block_no)}>
    ...
    <View style={{backgroundColor:"#fff", height:98, width:"100%", borderBottomWidth:1, borderBottomColor:"#b0acac", flex:100, flexDirection:"row",  padding:2, justifyContent:"space-between"}}>
      ...
      <View style={{ flex:30, flexDirection:"row", alignItems:"center", justifyContent:"space-around", }}>
          ...
          <Image source={require('../img/block.png')} style={{height:85, width:85}} resizeMode='contain'/>
          </View>
        ...
        <Text style={{color:"#be185d", fontSize:14}}> 
        সম্পন্ন {item.completed_bari}/{item.total_bari}
        </Text>
        </View> 
      </View>
      ...
      <View style={{flex:30, alignItems:"center", justifyContent:"space-around", flexDirection:"column"}}>
        ...
        <Image source={item.block_visit_status} style={{height:30, width:30}} resizeMode='contain'/>
      </View>
          ...
          <View
            ...
            <PieChart
              ...
              data={[
                ...
                {value: item.completed_bari, color: '#34d399'},
                {value: (item.total_bari-item.completed_bari), color: '#f472b6'}
              ]}
              ...
              centerLabelComponent={() => {
                return (
                  <View style={{justifyContent:"center", alignItems:"center"}}>
                    <Text style={{color: 'white', fontSize: 12}}>মোট বাড়ী </Text>
                    <Text style={{color: 'white', fontSize: 12}}>{item.total_bari}</Text>                    
                  </View>
                );
              }}
            />
...
export default memo(Block_list_data)
