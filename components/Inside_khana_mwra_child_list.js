...
import { StyleSheet, View, Text, FlatList, Alert, Image, ActivityIndicator, TouchableHighlight, Touchable, TouchableOpacity, Modal, ImageBackground } from 'react-native'
...
const Inside_khana_mwra_child_list = (props) => {
  ...
  const[data_update_tracker, SetData_update_tracker]=useState("0");
    ...
    const [loading, setLoading]=useState({
      ...
      const open_profile = async (mpid, Mem_SL, cStatus)=>{
      ...
      <View style={{backgroundColor:"#fff", height:90, width:"100%", borderBottomWidth:1, borderBottomColor:"#b0acac", flex:100, flexDirection:"row",  padding:10, justifyContent:"space-between"}}>
          ...
          <TouchableOpacity onPress={open_profile.bind(this,item.MPID, item.Member_SL, item.cStatus)} style={{flex:90, flexDirection:"row"}}>
        <View style={{ flex:35, flexDirection:"row", alignItems:"center", justifyContent:"space-between"}}>
        

          <ImageBackground source={require('../img/no_image.png')} resizeMode='contain' style={{flex:1.7, alignItems:"center",justifyContent:"center", marginLeft:15}}>
          <Image source={item.member_photo} style={{height:80, width:80, borderRadius:100, borderWidth:2, borderColor:"#0369a1"}}/>
          </ImageBackground>
  
          <View style={{flex:5, height:"100%", justifyContent:"center", marginLeft:10, marginLeft:18}}>
          ...
          <Text style={{color:"#404040"}}>{item.avatar}{item.gender}</Text>
      ...
      const SerachBar=()=>{
        return(
          ...
          <View style={{paddingLeft:5, flexDirection:"row", justifyContent:"space-between"}}>
            ...
            গর্বধারন ও সন্তান প্রসবের ইতিহাস  ।  সর্বমোট শিশু সংখ্যা  {totalRows.totalRowsCount}
            </Text>            
            </View>
    ...
    const get_khana_member_list_main_query= ()=>{
      ...
      db.transaction(tx=>{
        ...
        "case "+
         "when date(member.Entry_Date)<date('2022-03-01') then 'NotUpdated' "+
        ...
        "where "+ 
        "member.Mother_Line='"+props.pid+"' and member.Village_Code = '"+props.villageCode+"' and member.Bari_Code = '"+props.bari+"'  and member.HH_Code='"+props.hh+"' and member.Mem_Cstatus in ('1') "+
        "group by member.Mem_PID, member.Mem_SL "+
		"union all "+
"select "+
"'মৃত জন্ম, তাই কোন আইডি নেই' as 'FormattedMPID', "+
"'মৃত জন্ম, তাই কোন আইডি নেই' as 'FormattedMCID', "+
"'' as 'Mem_Enroll_Type', "+
"'' as 'Visit_Date', "+
"'' as 'MPID', "+
"'' as 'Member_SL', "+
...
"'' as 'Spouse', "+
"'' as 'is_MWRA', "+
"'মৃত জন্ম ' as 'Mem_Name', "+
"'মোট মৃত জন্ম '||SB_Num as 'Mem_Sex', "+
"Del_Date as 'Mem_DOB', "+
"'' as 'Mem_Cstatus', "+
"'' as 'path', "+
"'' as 'pregnent', "+
...
"from Preg_Outcome_Mother "+
    ...
    "union all "+
    "select "+
  "'গর্ভপাত, তাই কোন আইডি নেই' as 'FormattedMPID', "+
  "'গর্ভপাত, তাই কোন আইডি নেই' as 'FormattedMCID', "+
  "'' as 'Mem_Enroll_Type', "+
  "'' as 'Visit_Date', "+
  "'' as 'MPID', "+
  "'' as 'Member_SL', "+
  ...
  "from Preg_Outcome_Mother "+
      ...
      "Mem_PID = '"+props.pid+"' "+
            ...
            setTotalRows({totalRowsCount:length})
              ...
              if(length>0){
                ...
                for(let i= 0; i<length; i++){
                  ...
                  //  var PH_status = result.rows.item(i).PH_status; //PH_NotUpdated or PH_Updated
        ...
        if(Member_sex=="1"){
        avatar = <Image source={require('../img/man.png')} style={{height:20, width:20, marginTop:10}} resizeMode='contain'/>;
        gender = " পুরুষ"

        }
        else if(Member_sex=="2"){
        avatar = <Image source={require('../img/woman.png')} style={{height:20, width:20, marginTop:10}} resizeMode='contain'/>;
        gender = " মহিলা";
        }else{
          avatar = "";
          gender = <Text style={{fontSize:12, lineHeight:20}}>{Member_sex}</Text>;
        }
        ...
        if(IMG===null || IMG ===""){
          if(Member_sex=="1"){            
            ...
            member_photo = require('../img/woman_PIC.png');
              ...
              member_photo = require('../img/died.png');
            } 
        }
        else{
          member_photo = {uri:IMG+ "?" +image_updater};
                ...
                Alert.alert("গরবধারন ও সন্তান প্রসবের ইতিহাস", "এই MWRA এর কোন সন্তান লিস্টে নেই।")
                setLoading({loadingState:false})
              }
...
const update_preg_his=()=>{

  db.transaction(tx=>{
    tx.executeSql(
    "insert OR IGNORE Preg_his ("+
"LiveBirth,"+
"LB_Number,"+
"StillBirth,"+
"ST_Number,"+
...
{openProfile && (
<Member_profile mpid = {selectedMemberMPID} mem_sl = {selectedMemberMSL} toUpdate = {props.toUpdate} closeProfileModel_fun = {closeProfileModel}  pass_value_for_back_to_survey = {pass_value_for_back_to_survey_and_call_all_member} back_to_survey = {back_to_survey_question} villageCode = {props.villageCode} bari = {props.bari} hh = {props.hh} valuesForMwraSurvey={""}/>
)}
...
<Notification closeModel={closeModel} if_visible={modelView.visible} ChildError = {modelData.ChildError} EDDError = {modelData.EDDError} MWRAError = {modelData.MWRAError}/>
      ...
      keyExtractor={(MemberItems)=> MemberItems.MPID}
...
export default Inside_khana_mwra_child_list;
        ...
        // "select "+
        // "case when PH.PH_Mem_PID is null or PH.PH_Mem_PID = '' then 'PH_NotUpdated' else 'PH_Updated' end as 'PH_status', "+
        // "(substr(member.Mem_PID,1,3)||' '||substr(member.Mem_PID,4,2)||' '||substr(member.Mem_PID,6,3)||' '||substr(member.Mem_PID,9,3))'FormattedMPID', "+
        // "(substr(member.MCID,1,4)||' '||substr(member.MCID,5,3)||' '||substr(member.MCID,8,3)||' '||substr(member.MCID,11,3))'FormattedMCID', "+
        // "member.Mem_Enroll_Type, "+ 
        // "member.Visit_Date 'Visit_Date', "+
        ...
        //   "member.IMG "+
        // "from member "+
        // "left join (select Mem_PID 'PH_Mem_PID' from Preg_his) 'PH' on PH.PH_Mem_PID = member.Mem_PID "+
        // "left join "+
        // "(WITH a as( "+
        // "SELECT EDD, ROW_NUMBER() OVER (PARTITION BY pid ORDER BY CAST(MWRA_Survey.surveyNo AS int) DESC) 'rn' , pregnancyStatus, Entry_Date, pid, VisitOutCome from MWRA_Survey "+
        // "WHERE "+
