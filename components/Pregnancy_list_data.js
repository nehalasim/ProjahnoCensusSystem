...
import { useRef, ImageBackground, KeyboardAvoidingView, Keyboard, TouchableWithoutFeedback, Platform, TextInput, View, Text, FlatList, Alert, Image, ActivityIndicator, TouchableHighlight, Touchable, TouchableOpacity, Modal } from 'react-native'
...
const Pregnancy_list_data=(props)=> {
...
// const [defaultImage, setDefaultImage] = useState(require('../img/woman_PIC.png'));
      ...
      <TouchableOpacity onPress={open_profile.bind(this,item.MPID, item.Member_SL)} style={{flex:90, flexDirection:"row"}}>
        <View style={{ flex:35, flexDirection:"row", alignItems:"center", justifyContent:"space-between", height:"100%"}}>
        ...
        <ImageBackground source={require('../img/no_image.png')} style={{flex:1, alignItems:"center",justifyContent:"center", marginLeft:15}} resizeMode='contain'>
          ...
          <View style={{flex:2, height:"100%", justifyContent:"center", marginLeft:13}}>
          ...
          <Text style={{color:"#404040", fontSize:12}}>
          <Image source={require('../img/delivery-time.png')} style={{height:19, width:19}} resizeMode='contain'/> {item.EDD_Formatted}
          </Text>
      ...
      const [fullData, setFullData] = useState(MemberItems);
      const [filteredData, setFilteredData] = useState(MemberItems);
      const [searchText, setSearchText] = useState('');
    
      useEffect(() => {
        setFullData(MemberItems);
        setFilteredData(MemberItems);
      }, [MemberItems]);
      ...
      const filterData = (text) => {
        ...
        if (text) {
          ...
          const newData = fullData.filter(item => {
            const itemMemberSL = item.Member_SL ? item.Member_SL.toString() : '';
            const itemName = item.Mem_name ? item.Mem_name.toUpperCase() : '';
            const itemPID = item.MPID ? item.MPID.toString() : '';
            const itemMCID = item.MCID ? item.MCID.toString() : '';
            return itemMemberSL.indexOf(textData) > -1 ||
                   itemName.indexOf(textData) > -1 ||
                   itemPID.indexOf(textData) > -1 ||
                   itemMCID.indexOf(textData) > -1;
          });
      ...
      //   <InputField getValue={(text)=> setSearchText(text)} value={searchText} maxLength={40} placeholder={"সার্চ (নাম, সিআইডি, পিআইডি)"} imgLeft={"user"}/>
        
      //   <TouchableOpacity onPress={filterData.bind(this,searchText)}>
      //   <Text>
      //     S
      //   </Text>
      //   </TouchableOpacity>
    ...
    const get_khana_member_list_main_query=()=>{
      ...
      db.transaction(tx=>{
        ...
        "WHERE "+
        "VisitOutCome = '1'order by cast(MWRA_Survey.surveyNo as INT) desc )select * from a where rn = 1 "+
        ...
        "where member.cluster='"+props.cluster+"' and MWRA_Survey.pregnancyStatus = '4' and member.Mem_Cstatus in ('1') and cast(member.is_MWRA as INT) = 1 and "+
        "member.Village_Code||member.Bari_Code||member.HH_Code in (select village||bari||hh from HH where cStatus = '1' group by village||bari||hh) and "+
        "member.Village_Code||member.Bari_Code in (select village||bari from clusterDiv where cStatus = '1' and cluster = '"+props.cluster+"' group by village||bari) "+
        ...
        if(length>0){
          console.log(length);
                ...
                for(let i= 0; i<length; i++){
                   ...
                   var img_url = RNFS.DocumentDirectoryPath+"/PRFIMAGES/MEMBER/"+result.rows.item(i).Member_SL+".jpg";          
        ...
        if(IMG===null || IMG ===""){
          ...
          // RNFS.exists(RNFS.DocumentDirectoryPath+"/PRFIMAGES/MEMBER/"+MPID+".jpg")          
          // .then((exists)=> {
          //   if(!exists){
          //     if(Member_sex=="1"){            
          //       member_photo = require('../img/man_PIC.png');
          ...
          // console.log(RNFS.DocumentDirectoryPath+"/PRFIMAGES/MEMBER/"+MPID+".jpg");
...
{openProfile && (
<Member_profile mpid = {selectedMemberMPID} mem_sl = {selectedMemberMSL} fromMWRA_list = {true}  closeProfileModel_fun = {closeProfileModel}  pass_value_for_back_to_survey = {""} back_to_survey = {""}  valuesForMwraSurvey={valuesForMwraSurvey} byPass={true} />
//mwraVisit = {props.passedValues[15]} villageCode = {props.passedValues[6]} bari = {props.passedValues[8]} hh = {props.passedValues[10]}
)}
...
<View style={{width:"100%", padding:7, backgroundColor:"#fff", borderBottomWidth:1, borderBottomColor:"#b0acac", flexDirection:"row", justifyContent:"space-evenly", alignItems:"center"}}>
        <View style={{width:"80%"}}>
        <InputField getValue={(text)=> setSearchText(text)} value={searchText} maxLength={40} placeholder={"সার্চ (নাম, সিআইডি, পিআইডি)।"} imgLeft={"search"}/>
        </View>



        <TouchableOpacity onPress={filterData.bind(this,searchText)} style={{width:60, backgroundColor:"#e6e6e6", padding:10, borderRadius:5, alignItems:"center", justifyContent:"center"}}>
...
export default Pregnancy_list_data
